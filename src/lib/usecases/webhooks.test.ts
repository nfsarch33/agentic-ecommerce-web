...
import { deleteWebhookRegistration, loadWebhookSettings, registerWebhook, testWebhookDelivery } from "./webhooks";
...
describe("webhook usecases", () => {
  ...
  it("rejects blank URLs and registrations with no events before calling the adapter", async () => {
    const createWebhookImpl = vi.fn();

    await expect(
      registerWebhook({ baseUrl: "http://api.test", url: " ", eventTypes: ["product.approved"] }, { createWebhookImpl }),
    ).rejects.toThrow("url is required");
    await expect(
      registerWebhook({ baseUrl: "http://api.test", url: "https://hooks.example", eventTypes: [] }, { createWebhookImpl }),
