...
test("admin agents page manages sourcing, pricing, and schedule controls", async ({ page }) => {
  ...
  const scheduleToggle = page.getByLabel(/enable sourcing agent schedule/i);
  const wasEnabled = await scheduleToggle.isChecked();
  await scheduleToggle.click();
  await expect(page.getByRole("status")).toContainText(
    `Sourcing Agent schedule ${wasEnabled ? "disabled" : "enabled"}.`,
  );
});
